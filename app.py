...
st.markdown(
    ...
    :root {
        --brand-maroon: #800000;
        --brand-maroon-dark: #5e0b15;
        --brand-maroon-light: #a33a3a;
        --brand-blue: #0092D2;
        --brand-ink: #1f2937;
    }
    ...
    div[data-testid="stButton"] button[kind="primary"] {
        background: linear-gradient(90deg, var(--brand-maroon-dark), var(--brand-maroon-light)) !important;
    ...
    .person-table th {
        ...
        color: var(--brand-ink);
        background: linear-gradient(180deg, rgba(0, 146, 210, 0.09), rgba(0, 146, 210, 0.04));
        border-bottom: 1px solid rgba(0, 146, 210, 0.16);
    }
    .person-table td {
    ...
    .person-table tbody tr:hover td {
        background: linear-gradient(90deg, rgba(0, 146, 210, 0.05), rgba(0, 146, 210, 0.02));
    }
    ...
    .detail-link {
        ...
        height: 30px;
        border-radius: 10px;
        background: linear-gradient(135deg, rgba(128, 0, 0, 0.12), rgba(0, 146, 210, 0.14));
...
with refresh_col:
    ...
    st.caption(f"Last refreshed: {st.session_state.last_refresh_label}")
...
st.subheader("Person-wise Count")
person_summary_df = (
...
for row in person_summary_df.itertuples(index=False):
    ...
    row_values = {
        ...
        "name": full_name,
        "escaped_count": str(int(row.escaped_count)),
        "internal_count": str(int(row.internal_count)),
        "total_errors": str(int(row.total_errors)),
        "details": (
